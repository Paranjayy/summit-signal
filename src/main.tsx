...
const platforms: Platform[] = [
  ...
  { x: -2.8, y: 2.9, z: -1.3, width: 3.2, depth: 3, kind: 'container' },
  { x: 1.4, y: 5.6, z: -3.1, width: 3.4, depth: 2.6, kind: 'sign' },
  { x: 4.1, y: 8.6, z: -4.5, width: 2.8, depth: 3, kind: 'scaffold' },
  { x: 0.6, y: 11.5, z: -5.6, width: 3.6, depth: 2.4, kind: 'container' },
  { x: -3.3, y: 14.4, z: -6.8, width: 3.4, depth: 3.2, kind: 'cloud' },
  ...
  useFrame((state, delta) => {
    ...
    if (velocity.current.y < 0) {
      for (const p of platforms) {
        ...
        if (onTop && withinX && withinZ) { position.current.y = p.y + .34; velocity.current.y = 6.35; break }
      }
    }
