...
function CheckpointBeacon({ position, mode }: { position: [number, number, number]; mode: RunMode }) {
  const beacon = useRef<THREE.Group>(null)
  const color = mode === 'stormline' ? '#ff6b73' : mode === 'zenith' ? '#91e7ff' : '#f0b85f'
  useFrame((state) => { if (beacon.current) { beacon.current.rotation.y = state.clock.elapsedTime * .75; beacon.current.scale.setScalar(1 + Math.sin(state.clock.elapsedTime * 2.1) * .035) } })
  return <group ref={beacon} position={[position[0], position[1] + 1.15, position[2]]}>
    <pointLight color={color} intensity={2.2} distance={8} decay={2} />
    <mesh><cylinderGeometry args={[.035, .18, 2.3, 8]} /><meshStandardMaterial color={color} emissive={color} emissiveIntensity={2} transparent opacity={.72} /></mesh>
    <mesh rotation={[Math.PI / 2, 0, 0]}><torusGeometry args={[.72, .05, 8, 28]} /><meshStandardMaterial color={color} emissive={color} emissiveIntensity={1.8} transparent opacity={.82} depthWrite={false} /></mesh>
    <mesh rotation={[Math.PI / 2, 0, Math.PI / 2]}><torusGeometry args={[.45, .025, 8, 24]} /><meshStandardMaterial color="#fff0bf" emissive="#fff0bf" emissiveIntensity={1.2} transparent opacity={.62} depthWrite={false} /></mesh>
  </group>
}
...
function Player({ running, runId, mode, mutator, modifiers, heat, onProgress, onFall, onCollect, onArtifact, onGate, onGhostFrame, onLand, onMantle, onBurst, onGrapple, onCheckpoint, onViewChange, onHunterHit, onPhase, onComplete }: { running: boolean; runId: number; mode: RunMode; mutator: DailyMutator; modifiers: RunModifiers; heat: number; onProgress: (height: number) => void; onFall: () => boolean; onCollect: (index: number) => void; onArtifact: (index: number) => void; onGate: (index: number, airborne: boolean) => void; onGhostFrame: (frame: GhostFrame) => void; onLand: (perfect: boolean, shortcut: boolean) => void; onMantle?: () => void; onBurst: () => void; onGrapple: () => void; onCheckpoint: (height: number) => void; onViewChange: (view: 'third' | 'first') => void; onHunterHit: () => void; onPhase: () => void; onComplete: () => void }) {
  ...
  const phaseCooldown = useRef(0)
  const phaseVisual = useRef(0)
  const phaseLatch = useRef(false)
  ...
  useFrame((state, delta) => {
    ...
    const padSteer = gamepad ? THREE.MathUtils.clamp(gamepad.axes[0] ?? 0, -1, 1) : 0
    const padPace = gamepad ? THREE.MathUtils.clamp(-(gamepad.axes[1] ?? 0), -1, 1) : 0
    const steer = THREE.MathUtils.clamp(Number(Boolean(keys.current.d || keys.current.arrowright)) - Number(Boolean(keys.current.a || keys.current.arrowleft)) + (Math.abs(padSteer) > .16 ? padSteer : 0), -1, 1)
    const pace = THREE.MathUtils.clamp(Number(Boolean(keys.current.w || keys.current.arrowup || keys.current.keyw)) - Number(Boolean(keys.current.s || keys.current.arrowdown || keys.current.keys)) + (Math.abs(padPace) > .16 ? padPace : 0), -1, 1)
    const wantsJump = Boolean(keys.current[' '] || keys.current.space || keys.current.spacebar || gamepad?.buttons[0]?.pressed)
    ...
    if (wantsPhase && !phaseLatch.current && phaseCooldown.current <= 0) {
      phaseLatch.current = true
      phaseCooldown.current = 7
      phaseVisual.current = 1.25
    ...
    if (!grounded.current && velocity.current.y < 0) {
      for (const p of platforms) {
        ...
        if (onTop && withinX && withinZ) {
          ...
          const shortcutLanding = Math.abs(platformX) > 5.2
          position.current.y = p.y + .34; position.current.x = platformX; velocity.current.y = 0; grounded.current = true; cameraShake.current = Math.max(cameraShake.current, perfectLanding ? .2 : .14); onLand(perfectLanding, shortcutLanding)
    ...
    if (hunterCooldown.current <= 0 && phaseVisual.current <= 0) {
...
function World({ running, completed, mode, mutator, modifiers, biome, heat, ghostPath, runId, checkpointHeight, onProgress, onFall, collectedShards, collectedArtifacts, onCollect, onArtifact, onGate, onGhostFrame, onLand, onMantle, onBurst, onGrapple, onCheckpoint, onViewChange, onHunterHit, onPhase, onComplete }: { running: boolean; completed: boolean; mode: RunMode; mutator: DailyMutator; modifiers: RunModifiers; biome: BiomeId; heat: number; ghostPath: GhostFrame[]; runId: number; checkpointHeight: number; onProgress: (height: number) => void; onFall: () => boolean; collectedShards: Set<number>; collectedArtifacts: Set<number>; onCollect: (index: number) => void; onArtifact: (index: number) => void; onGate: (index: number, airborne: boolean) => void; onGhostFrame: (frame: GhostFrame) => void; onLand: (perfect: boolean, shortcut: boolean) => void; onMantle?: () => void; onBurst: () => void; onGrapple: () => void; onCheckpoint: (height: number) => void; onViewChange: (view: 'third' | 'first') => void; onHunterHit: () => void; onPhase: () => void; onComplete: () => void }) {
  ...
  const checkpointPlatform = platforms.reduce((best, platform) => Math.abs(platform.y - checkpointHeight) < Math.abs(best.y - checkpointHeight) ? platform : best, platforms[0])
...
function App() {
  ...
  const [perfectLands, setPerfectLands] = useState(0)
  const [shortcutLands, setShortcutLands] = useState(0)
  ...
  return <main className={heat > 70 ? 'heat-hot' : ''}>
    ...
    {running && <section className="route-map" aria-label="Vertical expedition map"><span>VERTICAL MAP</span><div className="route-map-track"><i style={{ bottom: `${progress}%` }} />{signalShards.map((shard, index) => <em key={`map-shard-${index}`} className={collectedShards.has(index) ? 'claimed' : ''} style={{ bottom: `${THREE.MathUtils.clamp(shard.y / courseHeight * 100, 4, 96)}%` }}>◆</em>)}<b style={{ bottom: '10%' }}>RUST</b><b style={{ bottom: '30%' }}>NEON</b><b style={{ bottom: '58%' }}>CLOUD</b><b style={{ bottom: '90%' }}>CORE</b></div><small>{biomeLabels[biome]} · {Math.floor(height * 10)}M / {Math.round(courseHeight * 10)}M</small></section>}
